...
T2x = [Theta(4),Theta(5)]*1e-3;
f = Theta(9);
M0b = Theta(6); M0f = (1-M0b); M0 = Theta(7)*[M0f M0b*(1-f) M0b*f];
K = Theta(8);
...
f = M0(3)/M0s; % Fraction of semisolid pool that is dipolar coupled.
...
M0tot = M0f+M0s;
KK = KK/M0tot; %%<--- Shaihan: these should not have units of magnetization.
...
Dphase = diag([-1 -1 1 1 1 1]); % Phase alternation matrix. %Dphase = diag([0 0 1 1 1 1]); % Spoiling matrix.
