...
Eff_FAdf = zeros(length(FA_var),length(df_var)); Con_FAdf = zeros(length(FA_var),length(df_var)); B1max_CalcTest = zeros(length(FA_var),length(df_var));
for ii = 1:length(FA_var)
    ...
    for jj = 1:length(df_var)
        ...
        [~,~,~,~,B1max_CalcTest(ii,jj)] = gen_CSMT_pulse_Diffnp(d2r(FA_var(ii)),ihMT_Params(3)*0.001,ihMT_Params(2)*0.001,B1rms,df_var(jj)*1000,3,ihMT_Params(5),ihMT_Params(6),'sigma',2);
...
nMB_var = 2:2:400; nSB_var = 2:2:400; 
